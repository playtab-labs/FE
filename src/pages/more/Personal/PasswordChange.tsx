...
export default function PasswordChange() {
  ...
  const handleSubmit = async () => {
    if (password === currentPassword) {
      showToast(t("personalChange.samePasswordError"));
      return;
    }
    try {
      ...
      if (res.data?.changeMyPassword.success) {
        ...
        showToast(t("personalChange.wrongCurrentPassword"));
      }
    } catch (e: any) {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <View className="flex-1">
        <Layout title={t("personalChange.passwordChangeAppBar")} showBack showCamera={false}>
          <View className="mt-6 gap-6 items-center">
            <Input
              label={t("personalChange.currentPasswordLabel")}
              placeholder={t("personalChange.currentPasswordPlaceholder")}
            ...
            <Input
              label={t("personalChange.newPasswordLabel")}
              description={
                isPasswordInvalid
                  ? t("setPassword.formatError")
                  : t("setPassword.subtitle")
              }
              placeholder={t("personalChange.newPasswordPlaceholder")}
              value={password}
            ...
            <Input
              label={t("personalChange.confirmNewPasswordLabel")}
              description={
                isMismatch ? t("setPassword.mismatch") : undefined
              }
              placeholder={t("personalChange.confirmNewPasswordPlaceholder")}
              value={confirm}
